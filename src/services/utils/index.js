...
export const convertToFloat = (price) => {
    const priceDouble = parseFloat(price.replace('.', '').replace(',', '.').replace(/[^0-9.]/g, ''));
...
export const orderListPrices = (prices) => {
    if(moment(prices[0].date) < moment(prices[prices.length-1].date)){
